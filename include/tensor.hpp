...
template <typename Ty> class Tensor;
...
template <typename Ty> const Tensor<Ty> operator*(Ty, const Tensor<Ty> &);

template <typename Ty> const Tensor<Ty> operator*(const Tensor<Ty> &, Ty);
